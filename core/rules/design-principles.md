---
id: governance/rules/design-principles
class: state
status: active
owner: governance-harness
updated: 2026-07-29
sources: [core/rules/design-principles.md]
---
...
### Requirement IDs
- Preserve the canonical IDs from requirements and their source specification.
- Numeric IDs such as `2.1` and domain IDs such as `LOT-U1` are both valid; never translate or renumber one form into another merely for this workflow.
- When creating new IDs, use the target repository's established convention consistently. If the repository has no convention, choose and document one before design.
- Every component, task, capability row, and traceability row must reference the same canonical ID.
