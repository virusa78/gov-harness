---
id: governance/rules/ears-format
class: state
status: active
owner: governance-harness
updated: 2026-07-29
sources: [core/rules/ears-format.md]
---
...
Write each specification consistently in its configured or established language. Trigger keywords may be translated when the condition, subject, and mandatory response remain unambiguous; do not mix languages inside one fixed pattern merely to satisfy a template.
...
## Project structural sample

When the target repository mandates a structural golden sample, use
`{{golden_sample}}`. This parameter names a project-local artifact; the core
rule never assumes its path.
